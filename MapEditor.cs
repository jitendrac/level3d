...
public class MapEditor : EditorWindow
{
	...
	public void OnGUI()
	{
		...
		{	
			if (! int.TryParse(GUILayout.TextField(sizeX.ToString()), out sizeX)) { sizeX = 0; }
			if (! int.TryParse(GUILayout.TextField(sizeY.ToString()), out sizeY)) { sizeY = 0; }
			if (GUILayout.Button("CREATE!") && sizeX > 0 && sizeY > 0)
			{
				...
				//loadResource ("AudioController", Vector3.zero, Vector3.zero);
			}
		}
	}
	...
	{
		if (Event.current.type == EventType.mouseDown){ button = Event.current.button; }
		if (Event.current.type == EventType.mouseUp)  { button = -1; }
		if (Event.current.type == EventType.keyDown && Event.current.keyCode == KeyCode.N) { editing = !editing; }
		if (button > -1 && editing)
		{			
